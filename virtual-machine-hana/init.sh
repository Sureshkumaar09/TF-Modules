...
UpdateNameServers(){
    ...
    echo -e "## Modifed by Automation Framework ######\ndomain dc0$dc.sf.priv\nsearch dc0$dc.sf.priv sf.priv openstack.${region}.cloud.sap" > $RESOLVE_CNF
    ...
    echo ${ip} ${hostname}.dc0${dc}.sf.priv ${hostname} > $HOSTS
}
...
Adduser(){
    ...
    #necessary so that terraform can be run multiple time and nothing would fail with the ccloud user
    echo "ccloud ALL=(ALL) NOPASSWD: ALL" > /etc/sudoers.d/sudo_root_for_ccloud
    chmod 440 /etc/sudoers.d/sudo_root_for_ccloud
...
Repos(){
    ...
    if [ -f /etc/os-release ]; then
        ...
        if [ $NAME == "SLES" ]; then
            ...
            zypper ar -G http://yum.dc0${dc}.sf.priv/hcm//common/yum/SLES/12.3/custom hcm_platform_common
            zypper ar -G http://yum.dc0${dc}.sf.priv/hcm//platform/yum/SLES/12.3 hcm_platform_services
